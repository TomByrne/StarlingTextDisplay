package starling.text.model.selection;
...
class Selection extends EventDispatcher
{
	...
	private function new(textDisplay:TextDisplay) 
	{
		...
		//textDisplay.addEventListener(Event.SELECT, OnInputChange);
	}
	...
	public function set(_begin:Int, _end:Int, _index:Int) 
	{
		...
		if (_index > textDisplay.value.length) _index = textDisplay.value.length;
		this._index = _index;
	...
	public function clear(?avoidEvent:Bool) 
	{
		...
		this.begin = null;
		this.end = null;
		if(!avoidEvent) dispatchEvent(new Event(Event.SELECT));
	}
	...
	{
		if (value < 0) value = 0;
		if (value > textDisplay.value.length) value = textDisplay.value.length;
		_index = value;
	...
	{
		_end = value;
		if (_end > textDisplay.value.length) _end = textDisplay.value.length;
		return _end;
	}
}