...
class HistoryModel
{
	...
	@:allow(starling.text.control.history) private static var stepPool:Array<HistoryStep> = [];
	...
	@:allow(starling.text.control.history)
	...
	{
		if (currStepIndex == -1 || currStepIndex>=history.length - 1) return;
	...
	function set_undoSteps(value:Int):Int 
	{
		...
		if (history.length > undoSteps){
			...
			if (currStepIndex < 0) currStepIndex = 0;
