...
class FormatParser
{
	...
	static private function findStartEnd(searchStartIndex:Int, nodes:Array<FormatNode>, plainText:String) 
	{
		...
		for (i in 0...nodes.length) 
		{
			...
			if (Std.is(nodes[i].value, Xml)) value = cast(nodes[i].value, Xml).nodeValue;
			else value = nodes[i].value;
	...
	static private function childEnd(nodes:Array<FormatNode>, plainText:String):Null<Int> 
	{
		...
		var value:String;
		if (Std.is(last.value, Xml)) value = cast(last.value, Xml).nodeValue;
	...
	public static function nodesToPlainText(value:Array<FormatNode>):String
	{
		...
		for (i in 0...value.length) 
		{
			if (value[i].value == null) output += nodesToPlainText(value[i].children);
			else output += value[i].value;
		}
		return output;
	}
...
/*	static function SortByStartAndEnd(a:FormatLength, b:FormatLength):Int
	...
	private static function createInputFormatStartTag(inputFormat:InputFormat):String
	{
		...
		if (inputFormat.color != null) atts += " color='#" + StringTools.hex(inputFormat.color, 6) + "'";
	...
	public static function copyCommonValues(copyTo:InputFormat, incompatible:InputFormat, copyFrom:InputFormat) 
	{
		...
		if (copyFrom.size != null) {
			if (copyTo.size == null && incompatible.size == null) copyTo.size = incompatible.size = copyFrom.size;
			else if (copyTo.size != copyFrom.size) copyTo.size = null;
		}
		if (copyFrom.face != null) {
			if (copyTo.face == null && incompatible.face == null) copyTo.face = incompatible.face = copyFrom.face;
			else if (copyTo.face != copyFrom.face) copyTo.face = null;
		}
		if (copyFrom.color != null) {
			if (copyTo.color == null && incompatible.color == null) copyTo.color = incompatible.color = copyFrom.color;
			else if (copyTo.color != copyFrom.color) copyTo.color = null;
		}
	...
	static public function removeAfterIndex(nodes:Array<FormatNode>, index:Int) 
	{
		...
		{
			var node:FormatNode = nodes[i];
			if (index <= node.startIndex) nodes.splice(i, 1);
...
class FormatNode
{
	...
	{
		#if js
			if (Std.is(v, Xml)) value = cast(v, Xml).nodeValue;
			else value = v;
		#else
			value = v;
		#end
